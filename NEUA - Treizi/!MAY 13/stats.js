...
function initializeCharts() {
    ...
    fileChart = new Chart(fileCtx, {
        type: 'doughnut',
        data: {
            labels: ['BSIT', 'BSCS', 'BSIS', 'BLIS', 'Others'],
            datasets: [{
                data: [0, 0, 0, 0, 0],
                backgroundColor: ['#6765E7', '#0092CC', '#FF794D', '#FF4D6F', '#aeaeae'],
                borderWidth: 0,
                spacing: 6,
                borderRadius: 20,
                hoverOffset: 12 
    ...
    userChart = new Chart(userCtx, {
        type: 'doughnut',
        data: {
            labels: ['Student', 'Alumni', 'Faculty', 'Dean', 'Coordinator', 'Pending'],
            datasets: [{
                data: [0, 0, 0, 0, 0, 0],
                backgroundColor: ['#FF9500', '#34C759', '#0092CC', '#6765E7', '#FF3B30', '#aeaeae'],
                borderWidth: 0,
                spacing: 6,
                borderRadius: 20,
                hoverOffset: 12
...
function loadChartData() {
    ...
    .then(([fileResponse, userResponse, pendingResponse, docResponse, returnedDocsResponse]) => {
        ...
        // Count active requests (Pending, Approved, Disapproved)
        ...
        // Remove counting returned docs for Damaged and Missing
        ...
        document.getElementById('userCount').textContent = 
            userData.student + userData.alumni + userData.faculty + userData.dean + userData.coordinator + userData.pending;
...
doc.text(`As of ${currentDate}`, 105, 28, { align: 'center' });
    ...
    fetchAdditionalStats().then(additionalData => {
        ...
        if (allTimeTopDocsData.length === 0) {
            allTimeTopDocsData.push(['No data', 'No documents borrowed', '-']);
        }
        
        doc.autoTable({
            startY: yPos,
            head: [
                [{ content: 'DOCUMENT REQUESTS (ALL TIME)', colSpan: 3, styles: { 
                    halign: 'center', 
                    fillColor: null, 
                    textColor: [0, 0, 0], 
                    fontStyle: 'bold',
        ...
        if (topDocsData.length === 0) {
            topDocsData.push(['No data', 'No documents borrowed in last 30 days', '-']);
        }
        
        doc.autoTable({
            startY: yPos,
            head: [
                [{ content: 'DOCUMENT REQUESTS (LAST 30 DAYS)', colSpan: 3, styles: { 
                    halign: 'center', 
                    fillColor: null, 
                    textColor: [0, 0, 0], 
                    fontStyle: 'bold',
...
function fetchAdditionalStats() {
    return new Promise((resolve) => {
        ...
        // Fetch returned documents to count completions and get top borrowed docs
        fetch('admin_dashboard.php?action=getReturnedDocs')
            ...
            .then(data => {
                if (data.success && data.requests) {
                    ...
                    data.requests.forEach(request => {
                        ...
                        if ((status === 'approved' || status === 'disapproved') && request.date_returned) {
