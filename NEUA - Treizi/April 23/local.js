...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (requestModal) {
        requestModal.addEventListener('show.bs.modal', function() {
            // Get all user info from sessionStorage
            const userName = sessionStorage.getItem('user_name');
            const userEmail = sessionStorage.getItem('user_email');
            ...
            // Get document info
            const documentId = document.getElementById('documentId')?.textContent || 'N/A';
            ...
            // Update request modal fields
            document.querySelector('#requestModal #userEmail').textContent = userEmail || 'Not available';
            document.querySelector('#requestModal #userName').textContent = userName || 'Not available';
            document.querySelector('#requestModal #userID').textContent = userId || 'Not available';
            document.querySelector('#requestModal #userUserType').textContent = userType || 'Not available';
...
// Add this function to handle populating the request modal
function populateRequestModal() {
    // Get document details from the main page
    const documentId = document.getElementById('documentId')?.textContent || 'N/A';
...
function createDocumentCard(doc) {
    ...
    card.innerHTML = `
        <div class="card-body">
            ...
            <div class="card-sub1" id="year"><i class="fa-regular fa-calendar"></i> ${escapeHtml(displayDate)}</div>
        </div>
    `;
