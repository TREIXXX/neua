...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action']) && $_GET['action'] === 'getUserDetails') {
    if (isset($_GET['email'])) {
        try {
            require_once 'config/db_connect.php'; // Make sure this points to your database connection file
...
function extractAbstract($text) {
    // Try to match either Abstract or Executive Summary, stopping at Table of Contents
    if (preg_match('/(?:EXECUTIVE\s+SUMMARY|Executive\s+Summary|ABSTRACT|Abstract)\s*\n\s*(.*?)(?=\s*(?:Table\s+of\s+Contents|TABLE\s+OF\s+CONTENTS))/si', $text, $matches)) {
