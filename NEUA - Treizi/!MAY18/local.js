// Dropdown data
// Define programs by department with display names
const programsByDepartment = {
    'Computing Studies': {
        'Computer Science': ['BSCS'],
        'Information Technology': ['BSIT'],
        'Information Systems': ['BSIS'],
        'Library Information Systems': ['BLIS'],
...
function setupDropdown() {
    ...
    if (yearDropdownBtn && yearDropdownMenu) {
        ...
        yearDropdownMenu.querySelectorAll('a').forEach((item) => {
            item.addEventListener('click', (event) => {
                ...
                if (text === 'Sort by Year') {
                    ...
                    // Don't do anything here as we'll handle the custom range separately
                    return;
                } else {
                    selectedYear = text;
                    sortByDate = false;
        ...
        // Add custom range inputs if they don't exist yet
        if (!document.getElementById('customYearRange')) {
            // Create custom range container
            const customRangeContainer = document.createElement('div');
            customRangeContainer.id = 'customYearRange';
            customRangeContainer.className = 'custom-year-range';
            customRangeContainer.style.padding = '8px';
            customRangeContainer.style.display = 'flex';
            customRangeContainer.style.flexDirection = 'column';
            customRangeContainer.style.gap = '8px';
            ...
            // Create from year input
            const fromYearInput = document.createElement('input');
            fromYearInput.type = 'number';
            fromYearInput.id = 'fromYear';
            fromYearInput.placeholder = '';
            fromYearInput.maxLength = 4;
            fromYearInput.min = 1900;
            fromYearInput.max = new Date().getFullYear();
            fromYearInput.style.width = '70px';
            fromYearInput.style.padding = '4px';
            
            // Create to year input
            const toYearInput = document.createElement('input');
            toYearInput.type = 'number';
            toYearInput.id = 'toYear';
            toYearInput.placeholder = '';
            toYearInput.maxLength = 4;
            toYearInput.min = 1900;
            toYearInput.max = new Date().getFullYear();
            toYearInput.style.width = '70px';
            toYearInput.style.padding = '4px';
            ...
            applyButton.addEventListener('click', function() {
                ...
                const toYear = parseInt(toYearInput.value);
                ...
                // Update button text
                yearDropdownBtn.innerHTML = `${fromYear} to ${toYear} <i class="fa-solid fa-chevron-down"></i>`;
                yearDropdownMenu.style.display = 'none';
            ...
            inputsContainer.appendChild(fromYearInput);
            inputsContainer.appendChild(document.createTextNode(' to '));
            inputsContainer.appendChild(toYearInput);
            
            customRangeContainer.appendChild(inputsContainer);
            customRangeContainer.appendChild(applyButton);
            
            // Add custom range option to the dropdown menu
            const customRangeLink = document.createElement('a');
            customRangeLink.href = '#';
            customRangeLink.textContent = 'Custom Range';
            customRangeLink.id = 'customRangeOption';
            
            // Add the custom range link and container at the end of the dropdown menu
            yearDropdownMenu.appendChild(customRangeLink);
            yearDropdownMenu.appendChild(customRangeContainer);
            
            // Initially hide the custom range inputs
            customRangeContainer.style.display = 'none';
            
            // Toggle custom range inputs when the link is clicked
            customRangeLink.addEventListener('click', function(e) {
                e.preventDefault();
                const isVisible = customRangeContainer.style.display === 'flex';
                customRangeContainer.style.display = isVisible ? 'none' : 'flex';
            });
        }
    }
    ...
    if (programDropdownBtn && programDropdownMenu) {
        ...
        programDropdownMenu.addEventListener('click', (event) => {
            if (event.target.tagName === 'A') {
                ...
                // Update program button text with display name instead of code
                programDropdownBtn.innerHTML = `${event.target.textContent} <i class="fa-solid fa-chevron-down"></i>`;
    ...
    function updateProgramDropdown() {
        ...
        if (selectedDepartment !== 'All Departments') {
            ...
            // For each program, add a dropdown option
            for (const [programName, programCodes] of Object.entries(programs)) {
                const link = document.createElement('a');
                link.href = '#';
                // Store all program codes as a comma-separated string in data-value
                link.setAttribute('data-value', programCodes.join(','));
                link.id = 'sort-program';
                link.textContent = programName;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('section-cics').style.display = 'none';
    document.getElementById('section-cas').style.display = 'none';
    document.getElementById('section-cea').style.display = 'none';

    // Get the user's department from sessionStorage
    const department = (sessionStorage.getItem('department') || '').toUpperCase();
    console.log('Department from sessionStorage:', department);

    // Show only the relevant section
    if (department === 'CICS') {
        document.getElementById('section-cics').style.display = '';
    } else if (department === 'CAS') {
        document.getElementById('section-cas').style.display = '';
    } else if (department === 'CEA' || department === 'COE') {
        document.getElementById('section-cea').style.display = '';
    } else {
        // If department is not set, show a message or default section
        console.warn('No valid department found in sessionStorage!');
    }
});
...
function createDocumentCard(doc) {
    ...
    if (doc.creationDate) {
        ...
        const yearMatch = doc.creationDate.match(/\b(19|20)\d{2}\b/);
        if (yearMatch) {
            displayDate = yearMatch[0];
        } else {
