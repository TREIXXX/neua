    ...
    fetch('admin_dashboard.php?action=getApprovedUsers')
        ...
        .then(data => {
            if (data.success) {
                ...
                // Update user count in dashboard stats
                document.querySelector('.dashboard-stats .stat:nth-child(2) h3').textContent = allUserRecords.length;
    ...
    .then(([approvedData, pendingData]) => {
        if (pendingData.success) {
            ...
            pendingData.pendingUsers.forEach((user, index) => {
                ...
                // Add debug logging for certificate path
                console.log('Certificate path for user:', user.email, row.certificate_url);
...
function toggleUserStatus(userId, isCurrentlyActive) {
    ...
    showConfirmDialog({
        ...
        cancelText: 'Cancel',
        dialogType: isCurrentlyActive ? 'danger' : 'default',
        onConfirm: () => {
            const formData = new FormData();
...
function viewPendingUser(name, email, ID, userType, certificateUrl) {
    ...
    // Show certificate in overlay if available and user is not faculty
    if (certificateUrl && certificateUrl !== 'undefined' && userType.toLowerCase() !== 'faculty') {
        showCertificateOverlay(certificateUrl);
    }
}
