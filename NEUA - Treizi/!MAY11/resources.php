...
function searchDocuments($searchQuery, $page = 1, $selectedYear = '', $sortByDate = false, $category = 'Any Field', $selectedDepartment = '', $selectedProgram = '') {
    global $pdo;
    $resultsPerPage = 20;
    ...
    try {
        ...
        if (!empty($selectedYear)) {
            if (strpos($selectedYear, 'Since') !== false) {
                ...
                $params[':year'] = $year;
            } else if (strpos($selectedYear, '-') !== false) {
                // Handle custom year range (format: fromYear-toYear)
                list($fromYear, $toYear) = explode('-', $selectedYear);
                $baseQuery .= " AND year >= :fromYear AND year <= :toYear";
        ...
        // Department filter mapping with program codes
        $departmentCourses = [
            'Computing Studies' => ["BSCS", "BSIT", "BSIS", "BLIS", "BSEMC"],
            'Arts & Sciences' => ["BA Econ", "BA PolSci", "Psych", "Pub Adm."],
            'Engineering & Architecture' => ["BSCE", "BSEE", "BSME", "BS Arch"]
        ];

        // Add department filter if selected
        if (!empty($selectedDepartment) && $selectedDepartment !== 'All Departments') {
            if (isset($departmentCourses[$selectedDepartment])) {
                $courses = $departmentCourses[$selectedDepartment];
                $placeholders = array_map(function($i) { return ":course$i"; }, array_keys($courses));
                $baseQuery .= " AND course IN (" . implode(", ", $placeholders) . ")";
                foreach ($courses as $i => $course) {
                    $params[":course$i"] = $course;
                }
            }
        }

        // Add program filter if selected (after department filter)
        if (!empty($selectedProgram) && $selectedProgram !== 'All Programs') {
            // Check if the program contains multiple codes (comma-separated)
            if (strpos($selectedProgram, ',') !== false) {
                $programCodes = explode(',', $selectedProgram);
                $placeholders = array_map(function($i) { return ":program$i"; }, array_keys($programCodes));
                $baseQuery .= " AND course IN (" . implode(", ", $placeholders) . ")";
                foreach ($programCodes as $i => $code) {
                    $params[":program$i"] = $code;
                }
            } else {
...
$searchResults = searchDocuments($searchQuery, $page, $selectedYear, $sortByDate, $category, $selectedDepartment, $selectedProgram);
