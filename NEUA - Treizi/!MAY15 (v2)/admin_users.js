...
function updateDisapprovedUsersPagination() {
    const total = filteredDisapprovedUsers.length;
    const pages = Math.ceil(total / disapprovedRowsPerPage);
    const pagination = document.getElementById('disapprovedUsersTablePagination');
    ...
    prevBtn.disabled = disapprovedCurrentPage === 0;
    prevBtn.onclick = () => {
        if (disapprovedCurrentPage > 0) {
    ...
    const startPage = Math.max(0, Math.min(disapprovedCurrentPage - Math.floor(maxPageButtons / 2), pages - maxPageButtons));
    const endPage = Math.min(pages, startPage + maxPageButtons);
    
    for (let i = startPage; i < endPage; i++) {
        const pageBtn = document.createElement('button');
        pageBtn.textContent = i + 1;
        pageBtn.className = i === disapprovedCurrentPage ? 'active' : '';
        pageBtn.onclick = () => {
            disapprovedCurrentPage = i;
            renderDisapprovedUsersTable();
            updateDisapprovedUsersPagination();
        };
    ...
    nextBtn.disabled = disapprovedCurrentPage >= pages - 1;
    nextBtn.onclick = () => {
        if (disapprovedCurrentPage < pages - 1) {
            disapprovedCurrentPage++;
            renderDisapprovedUsersTable();
            updateDisapprovedUsersPagination();
        }
...
function exportDisapprovedUsers() {
    const headers = ['Name', 'Email', 'ID', 'User Type', 'Certificate'];
...
function updateDeactivatedUsersPagination() {
    ...
    const totalPages = Math.ceil(filteredDeactivatedUsers.length / deactivatedRowsPerPage);
    ...
    // Previous button
    paginationHTML += `<button class="page-btn ${deactivatedCurrentPage === 0 ? 'disabled' : ''}" ${deactivatedCurrentPage === 0 ? 'disabled' : ''} onclick="changePagination('deactivatedUsersTable', ${deactivatedCurrentPage - 1})">Previous</button>`;
    ...
    for (let i = startPage; i <= endPage; i++) {
        paginationHTML += `<button class="page-btn ${i === deactivatedCurrentPage ? 'active' : ''}" onclick="changePagination('deactivatedUsersTable', ${i})">${i + 1}</button>`;
    }
    
    // Next button
    paginationHTML += `<button class="page-btn ${deactivatedCurrentPage === totalPages - 1 ? 'disabled' : ''}" ${deactivatedCurrentPage === totalPages - 1 ? 'disabled' : ''} onclick="changePagination('deactivatedUsersTable', ${deactivatedCurrentPage + 1})">Next</button>`;
    
    paginationContainer.innerHTML = paginationHTML;
}

// Function to change pagination page for deactivated users
function changePagination(tableId, page) {
    if (tableId === 'deactivatedUsersTable') {
