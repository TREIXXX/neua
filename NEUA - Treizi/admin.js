...
window.onload = function() {
    ...
    // Initialize auto-logout if the function exists
    ...
    if (nextState !== 'default') {
        ...
        records.sort((a, b) => {
            ...
            switch (tableId) {
                ...
                case 'userRecordsTable':
                    const userProps = ['name', 'email', 'ID', 'userType', 'program'];
                    aValue = a[userProps[columnIndex - 1]] || '';
                    bValue = b[userProps[columnIndex - 1]] || '';
                    
                    // Special handling for userType column to follow the specified order
                    if (columnIndex === 4) { // userType column
                        // Define the sort order: dean > coordinator > faculty > student > alumni
                        const userTypeOrder = {
                            'dean': 1,
                            'coordinator': 2,
                            'faculty': 3,
                        ...
                        // Get the sort values (default to highest number if not found)
                        const aOrder = userTypeOrder[aValue.toString().toLowerCase()] || 999;
                        const bOrder = userTypeOrder[bValue.toString().toLowerCase()] || 999;
                        
                        // Sort by the defined order
                        return nextState === 'asc' ? aOrder - bOrder : bOrder - aOrder;
                    }
                    break;
...
function renderTable(tableId, records) {
    ...
    records.forEach((record, index) => {
        ...
        if (['fileRecordsTable', 'userRecordsTable', 'accountRequestsTable'].includes(tableId)) {
            ...
            // For userRecordsTable, determine if checkbox should be enabled based on department
            if (tableId === 'userRecordsTable') {
                ...
                // Check if user's program belongs to the current department
                const userProgram = record.program || '';
                const userDepartment = programToDepartment[userProgram] || '';
                const isSameDepartment = userDepartment === window.currentUserDepartment;
                const isAdmin = window.currentUserType === 'admin';
                
                // Handle special case for deans with "All Programs"
                let isDeanInSameDept = false;
                if (userProgram === 'All Programs' && record.department) {
                    isDeanInSameDept = record.department === window.currentUserDepartment;
                }
                
                const canModify = isAdmin || isSameDepartment || isDeanInSameDept;
                
                // Determine if "See All Programs" is currently active - we need to check the checkbox state
                const seeAllCheckbox = document.getElementById('seeAllUserPrograms');
                const isSeeAllActive = seeAllCheckbox && seeAllCheckbox.checked;
                
                if (isSeeAllActive && !canModify) {
                    // Disable checkbox with explanation tooltip
                    checkboxCell.innerHTML = `<input type="checkbox" disabled title="You can only select users in your department" style="opacity: 0.6;">`;
                } else {
                    // Normal enabled checkbox
                    checkboxCell.innerHTML = `<input type="checkbox" onchange="handleCheckboxClick('${tableId}', this)">`;
                }
            } else {
                // For other tables, add normal checkbox
                checkboxCell.innerHTML = `<input type="checkbox" onchange="handleCheckboxClick('${tableId}', this)">`;
            }
        }
...
-->`;
                ...
                case 'userRecordsTable':
                    ...
                    ['name', 'email', 'ID', 'userType', 'program'].forEach((prop, index) => {
                        const cell = row.insertCell();
                        const value = record[prop] || '';
                    ...
                    // Determine if user's program belongs to the current department
                    const isAdmin = window.currentUserType === 'admin';
                    ...
                    const userDepartment = programToDepartment[userProgram] || '';
                    const isSameDepartment = userDepartment === window.currentUserDepartment;
                    const canDeactivate = isAdmin || isSameDepartment;
                    
                    // Dean can deactivate any user in their department
                    // Also handle special case for "All Programs" which deans have
                    if (window.currentUserType === 'dean' && userProgram === 'All Programs') {
                        // If the user is a dean (with All Programs), check if they belong to the same department as the logged-in dean
                        const canDeanDeactivateDean = record.department === window.currentUserDepartment;
                        if (canDeanDeactivateDean) {
                            canDeactivate = true;
                        }
                    }
                    
                    // User ID for data attribute
                    const userId = record.ID || '';
                    
                    // Create view action
                    let userActionsMenu = `
                        <div class="action-item" onclick="viewUser('${record.name || ''}', '${record.email || ''}', '${userId}', '${record.userType || ''}', '${record.created_at || ''}', '${record.certificate_url || ''}', '${record.created_by || ''}')">
                            <i class="fas fa-eye"></i>View
                        </div>`;
                    
                    // Add deactivate/activate option based on permission and current status
                    if (canDeactivate) {
                        userActionsMenu += `
                            <div class="action-item ${isActive ? 'deactivate-action' : 'activate-action'}" onclick="toggleUserStatus('${userId}', ${isActive})">
                                <i class="fas ${isActive ? 'fa-circle-minus' : 'fa-circle-check'}"></i>${isActive ? 'Deactivate' : 'Activate'}
                            </div>`;
                    } else if (isActive) {
                        // If user can't deactivate, show disabled option with title
                        userActionsMenu += `
                            <div class="action-item disabled" title="You can only deactivate users in your department">
                                <i class="fas fa-circle-minus"></i>Deactivate
                            </div>`;
                    } else {
                        // If already deactivated and not in same department, don't show option
                    }
                    
                    userActionsCell.innerHTML = `
                        <div class="actions-dropdown" data-user-id="${userId}">
            ...
            case 'accountRequestsTable':
                ...
                ['name', 'email', 'ID', 'userType', 'program'].forEach((prop, index) => {
                    const cell = row.insertCell();
                    const value = record[prop] || (prop === 'program' ? record.department || '' : '');
...
function exportUserRecords() {
    const headers = ['Name', 'Email', 'ID', 'User Type', 'Program'];
    
    const data = filteredUserRows.length > 0 ? filteredUserRows : allUserRecords;
    
    // Map data to array of just the fields we want to export
    ...
    downloadCSV(headers, exportData, 'user_records.csv');
}
...
function exportReturnedDocs() {
    const headers = ['Reference No.', 'Doc ID', 'Title', 'Requester Name', 'Email', 'Date Sent'];
...
function exportDisapprovedDocs() {
    const headers = ['Reference No.', 'Doc ID', 'Title', 'Requester Name', 'Email', 'Date Disapproved'];
...
function toggleSelectAll(tableId, checkbox) {
    ...
    const checkboxes = table.querySelectorAll('tbody input[type="checkbox"]:not([disabled])');
    
    // Check/uncheck all non-disabled checkboxes
...
function updateDeleteButtonVisibility(tableId) {
    ...
    const deleteButton = document.querySelector(`.btn-delete[onclick*="${tableId}"]`);
...
// Make sure to replace any existing select all handlers with this updated function
document.addEventListener('DOMContentLoaded', function() {
    // Set up the event handler for the select all checkbox in user records table
    const selectAllUsersCheckbox = document.querySelector('#userRecordsTable thead input[type="checkbox"]');
    if (selectAllUsersCheckbox) {
        selectAllUsersCheckbox.addEventListener('change', function() {
            toggleSelectAll('userRecordsTable', this);
        });
    }
});
...
const filterState = {
    ...
    users: {
        department: new Set(),  // Add department filter state (replaces program filter)
        userType: new Set(),
        status: '',
        withRequests: false
    }
...
function setupFilterHandlers() {
    ...
    document.querySelectorAll('[value="bsit"], [value="bscs"], [value="bsis"], [value="blis"], [value="ba_econ"], [value="ba_polsci"], [value="bs_psych"], [value="bpa"], [value="bsce"], [value="bsee"], [value="bsme"], [value="bs_arch"]').forEach(checkbox => {
    ...
    const seeAllProgramsCheckbox = document.getElementById('seeAllPrograms');
    if (seeAllProgramsCheckbox) {
        seeAllProgramsCheckbox.addEventListener('change', function() {
            ...
            if (this.checked) {
                ...
                filterState.files.program.clear();
            }
            
            // Force a reload of file records from the server with appropriate see_all parameter
            loadFileRecords();
        });
    }
    ...
    // Call the function to set up See All Programs handler for users
    setupSeeAllUserProgramsHandler();
}
...
function applyUserFilters() {
    ...
    filteredUserRows = allUserRecords.filter(record => {
        ...
        // Department filter
        const matchesDepartment = filterState.users.department.size === 0 || 
            filterState.users.department.has(record.department);
        ...
        return matchesSearch && matchesDepartment && matchesType && matchesStatus && matchesRequests;
    });
 ...
 * @param {boolean} showCancelButton - Whether to show the cancel button (optional)
 * @param {boolean} isHTML - Whether the message is HTML (optional)
...
function showConfirmDialog({ 
    ...
    isHTML = false, // Add parameter to handle HTML content
    onConfirm = null
}) {
    return new Promise((resolve) => {
        ...
        if (isHTML) {
            // Use innerHTML if isHTML flag is true
            messageEl.innerHTML = message;
        } else {
            // Use textContent for plain text (more secure)
            messageEl.textContent = message;
        }
        
        confirmBtn.textContent = confirmText;
        
        // Show/hide cancel button based on showCancelButton parameter
        if (showCancelButton) {
...
function requestDocument(documentId) {
    // First check if the document is already reserved
    if (checkIfDocumentReserved(documentId)) {
        showAlert('Someone already requested this file', 'warning');
        return;
    }
    
    // Get the document details (title) from the file records
    let docTitle = '';
    const document = allFileRecords.find(record => record.documentId === documentId);
    if (document) {
        docTitle = document.title || document.fileName || 'Unknown Title';
    }
    
    // Create content for the confirmation modal including Doc ID and Title
    const confirmMessage = `
        <p>Are you sure you want to request this document?</p>
    ...
    showConfirmDialog({
        ...
        confirmText: 'Request',
        cancelText: 'Cancel',
        isHTML: true, // Add this parameter to support HTML content
        onConfirm: () => {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    showAlert(data.message || 'Error requesting document', 'warning');
                }
            })
            .catch(error => {
                console.error('Error:', error);
