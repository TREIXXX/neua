...
function loadAccountRequests() {
    ...
    .then(([approvedData, pendingData]) => {
        ...
        if (pendingData.success) {
            ...
            // Update total user count in dashboard
            const totalUsers = (approvedData.success ? approvedData.approvedUsers.length : 0) + 
                             allAccountRequests.length;
            document.querySelector('.dashboard-stats .stat:nth-child(3) h3').textContent = totalUsers;
...
function renderDisapprovedUsersTable() {
    ...
    paginatedRecords.forEach(record => {
        ...
        checkboxCell.className = 'checkbox-cell text-center';
        checkboxCell.innerHTML = `<input type="checkbox" class="disapproved-checkbox" onchange="updateDisapprovedCheckboxes()">`;
        ...
        typeCell.innerHTML = `<span class="user-type ${userTypeClass}" style="display: inline-block; padding: 4px 16px; border-radius: 8px; font-size: 0.875rem; font-weight: 500; min-width: 80px; color: #FF9500; background-color: #FFEACC;">${record.userType}</span>`;
        
        // Reason cell
        const reasonCell = row.insertCell();
        reasonCell.className = 'text-center';
        reasonCell.innerHTML = `<button class="btn btn-sm btn-outline-secondary" onclick="viewRejectionReason('${record.name}', '${record.email}', '${record.ID}', '${record.userType}', '${encodeURIComponent(record.reject_reason || 'No reason provided')}')">
...
// Function to handle changing pagination for disapproved users table
function updateDisapprovedUsersPagination() {
    const table = document.getElementById('disapprovedUsersTable');
    if (!table) return;
    
    const settings = tableSettings['disapprovedUsersTable'] || { rowsPerPage: 20, currentPage: 1 };
    ...
    prevButton.onclick = () => changePagination('disapprovedUsersTable', settings.currentPage - 1);
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        pageButton.onclick = () => changePagination('disapprovedUsersTable', i);
    ...
    nextButton.onclick = () => changePagination('disapprovedUsersTable', settings.currentPage + 1);
...
// Function to change pagination page for any table
...
// Function to handle pagination for deactivated users table
function updateDeactivatedUsersPagination() {
    const table = document.getElementById('deactivatedUsersTable');
    if (!table) return;
    
    const settings = tableSettings['deactivatedUsersTable'] || { rowsPerPage: 20, currentPage: 1 };
    const tbody = table.querySelector('tbody');
    const rows = tbody.querySelectorAll('tr');
    const totalPages = Math.ceil(rows.length / settings.rowsPerPage);
    ...
    prevButton.innerHTML = '<i class="fa-solid fa-chevron-left"></i>';
    prevButton.disabled = settings.currentPage === 1;
    prevButton.onclick = () => changePagination('deactivatedUsersTable', settings.currentPage - 1);
    paginationContainer.appendChild(prevButton);
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        pageButton.textContent = i;
        pageButton.className = i === settings.currentPage ? 'active' : '';
        pageButton.onclick = () => changePagination('deactivatedUsersTable', i);
        paginationContainer.appendChild(pageButton);
    }
    ...
    nextButton.innerHTML = '<i class="fa-solid fa-chevron-right"></i>';
    nextButton.disabled = settings.currentPage === totalPages;
    nextButton.onclick = () => changePagination('deactivatedUsersTable', settings.currentPage + 1);
    paginationContainer.appendChild(nextButton);
...
// Update the renderDeactivatedUsersTable function to include pagination
function renderDeactivatedUsersTable() {
    const tableBody = document.querySelector('#deactivatedUsersTable tbody');
    if (!tableBody) return;
    
    const searchTerm = document.getElementById('deactivatedUserSearch')?.value.toLowerCase() || '';
    
    // Filter deactivated users by search term
    ...
    if (filteredDeactivatedUsers.length === 0) {
        ...
        cell.className = 'text-center';
        cell.innerHTML = 'No deactivated users found';
        
        // Reset pagination
        document.getElementById('deactivatedUsersTablePagination').innerHTML = '';
        return;
    }
    ...
    filteredDeactivatedUsers.forEach(user => {
        ...
        userTypeSpan.className = `user-type ${(user.userType || '').toLowerCase()}`;
        userTypeSpan.textContent = user.userType;
        userTypeCell.appendChild(userTypeSpan);
        
        const programCell = row.insertCell();
        programCell.textContent = user.program;
        
        const reasonCell = row.insertCell();
        reasonCell.textContent = user.reason || 'N/A';
        
        const actionsCell = row.insertCell();
        const reactivateBtn = document.createElement('button');
        reactivateBtn.className = 'btn btn-sm btn-outline-success';
        reactivateBtn.innerHTML = '<i class="fas fa-user-check"></i>';
        reactivateBtn.setAttribute('title', 'Reactivate User');
        reactivateBtn.onclick = () => reactivateUser(user.ID);
...
// Update the renderDisapprovedUsersTable function to include pagination
function renderDisapprovedUsersTable() {
    const tableBody = document.querySelector('#disapprovedUsersTable tbody');
    if (!tableBody) return;
    
    const searchTerm = document.getElementById('disapprovedUserSearch')?.value.toLowerCase() || '';
    
    // Filter disapproved users by search term
    ...
    if (filteredDisapprovedUsers.length === 0) {
        ...
        cell.className = 'text-center';
        cell.innerHTML = 'No disapproved users found';
        
        // Reset pagination
        document.getElementById('disapprovedUsersTablePagination').innerHTML = '';
        return;
    }
    
    // Render rows
    filteredDisapprovedUsers.forEach(user => {
        const row = tableBody.insertRow();
        row.setAttribute('data-user-id', user.ID);
        
        const checkboxCell = row.insertCell();
        checkboxCell.className = 'text-center';
        checkboxCell.innerHTML = `<input type="checkbox" class="disapproved-user-checkbox" onchange="updateDisapprovedCheckboxes()">`;
        ...
        const userTypeCell = row.insertCell();
        userTypeCell.className = 'text-center';
        const userTypeSpan = document.createElement('span');
        userTypeSpan.className = `user-type ${(user.userType || '').toLowerCase()}`;
        userTypeSpan.textContent = user.userType;
        userTypeCell.appendChild(userTypeSpan);
        ...
        // Add "View Reason" button
        const viewReasonBtn = document.createElement('button');
        viewReasonBtn.className = 'btn btn-sm btn-outline-secondary';
        viewReasonBtn.innerHTML = 'View Reason';
        viewReasonBtn.onclick = () => viewRejectionReason(user.name, user.email, user.ID, user.userType, user.reason);
        reasonCell.appendChild(viewReasonBtn);
    });
...
function renderDeactivatedUsersTable() {
    ...
    paginatedRecords.forEach(record => {
        ...
        actionsCell.innerHTML = `
            <button class="btn btn-sm btn-success" onclick="reactivateUser('${record.ID}')">
