...
function sortTable(tableId, columnIndex) {
    ...
    } else if (tableId === 'documentRequestsTable') {
        searchActive = document.getElementById('documentRequestSearch').value !== '';
        records = searchActive ? [...filteredDocumentRequests] : [...allDocumentRequests];
    } else {
        return;
    }
    ...
    } else if (tableId === 'documentRequestsTable' && originalDocumentRequests.length === 0) {
        originalDocumentRequests = [...allDocumentRequests];
    }
        ...
        records.sort((a, b) => {
            ...
            switch (tableId) {
                ...
                case 'accountRequestsTable':
                    ...
                    break;
                case 'documentRequestsTable':
                    const documentProps = ['reference', 'docId', 'title', 'borrower', 'dateReleased', 'status'];
                    aValue = a[documentProps[columnIndex - 1]] || '';
                    bValue = b[documentProps[columnIndex - 1]] || '';
                    break;
            }
    ...
    } else if (tableId === 'documentRequestsTable' && searchActive) {
            filteredDocumentRequests = records;
    }
...
function changeRowsPerPage(tableId, newValue) {
    ...
    switch (tableId) {
        ...
        case 'accountRequestsTable':
            ...
            break;
        case 'documentRequestsTable':
            currentData = filteredDocumentRequests.length > 0 ? filteredDocumentRequests : allDocumentRequests;
            break;
    }
...
function loadDocumentRequests() {
    ...
    const documentRequests = [
        ...
        },
        {
            ...
            title: 'AI World: LLMS',
            borrower: '402379',
            dateReleased: '11/15/25 8:30 AM',
            status: 'released'
        },
        {
            reference: '25006',
            docId: 'CSU-2110-25',
            title: 'Web3: Blockchain Tec...',
            borrower: '19-12345-678',
            dateReleased: '12/10/25 03:15 PM',
            status: 'overdue'
        }
    ];
    ...
    documentRequests.forEach(request => {
        ...
        row.innerHTML = `
            <td><input type="checkbox" onchange="handleCheckboxClick('documentRequestsTable', this)"></td>
            <td>${request.reference}</td>
            <td>${request.docId}</td>
            <td>${request.title}</td>
            <td>${request.borrower}</td>
            <td>${request.dateReleased}</td>
            <td><span class="status-badge ${request.status}">${request.status.charAt(0).toUpperCase() + request.status.slice(1)}</span></td>
            <td class="actions">
                <div class="actions-dropdown">
                    <button type="button" onclick="toggleActionsMenu(this)">
                        <i class="fas fa-ellipsis-v"></i>
                    </button>
                    <div class="actions-menu">
                        <div class="action-item" onclick="viewDocumentRequest('${request.reference}')">
                            <i class="fas fa-eye"></i> View
                        </div>
                        <div class="action-item" onclick="markAsComplete('${request.reference}')">
                            <i class="fas fa-check"></i> Complete
                        </div>
                        <div class="action-item delete-action" onclick="deleteDocumentRequest('${request.reference}')">
            ...
            case 'accountRequestsTable':
                ...
                reqActionsCell.innerHTML = `
                    <div class="actions-dropdown" data-request-id="${record.requestId}">
                        <button class="btn btn-sm" onclick="acceptRequest('${record.requestId}')">
                ...
                break;
            case 'documentRequestsTable':
                // Add cells for document requests with copy functionality
                ['reference', 'docId', 'title', 'borrower', 'dateReleased', 'status'].forEach((prop, index) => {
                    const cell = row.insertCell();
                    const value = record[prop] || '';
                ...
                docActionsCell.innerHTML = `
                    <div class="actions-dropdown" data-document-id="${record.reference}">
                        <button class="btn btn-sm" onclick="toggleActionsMenu(this)">
                            <i class="fas fa-ellipsis"></i>
                        </button>
                        <div class="actions-menu">
                            <div class="action-item" onclick="viewDocumentRequest('${record.reference}')">
                                <i class="fas fa-eye"></i>View
                            </div>
                            <div class="action-item" onclick="editRow(this.closest('.actions-dropdown'))">
                                <i class="fas fa-edit"></i>Edit
...
function refreshDocumentRequestsRecords() {
    ...
    // Reset sorting (clear arrow indicators)
    document.querySelectorAll('#documentRequestsTable th span').forEach(span => {
        span.classList.remove('asc', 'desc');
    });
...
document.addEventListener('DOMContentLoaded', () => {
    const tables = ['fileRecordsTable', 'userRecordsTable', 'accountRequestsTable', 'documentRequestsTable'];
...
function exportDocumentRequestsRecords() {
    ...
    const rows = Array.from(document.querySelectorAll('#documentRequestsTable tbody tr')).map(row => {
        return Array.from(row.children)
            .slice(1, 7) // Skip checkbox and actions columns
    ...
    if (confirm(`Are you sure you want to delete ${selectedRows.length} ${recordType}?`)) {
        const promises = Array.from(selectedRows).map(checkbox => {
            ...
            if (tableId === 'fileRecordsTable') {
                ...
                const documentId = row.querySelector('[data-document-id]').getAttribute('data-document-id');
                formData.append('action', 'rejectDocument');
...
function applyAccountFilters() {
    ...
    // Re-apply current sort if exists
    const currentSortHeader = document.querySelector('#accountRequestsTable th span.asc, #accountRequestsTable th span.desc');
    if (currentSortHeader) {
        const columnIndex = parseInt(currentSortHeader.id.replace('accountRequestsTableHeader', ''));
        const isAsc = currentSortHeader.classList.contains('asc');
        
        filteredAccountRequests.sort((a, b) => {
            // ... apply same sorting logic as in sortTable ...
            const props = ['name', 'email', 'ID', 'userType'];
            let aValue = a[props[columnIndex - 1]] || '';
            let bValue = b[props[columnIndex - 1]] || '';
...
function applyDocumentFilters() {
    ...
    filteredDocumentRequests = allDocumentRequests.filter(record => {
        ...
        // Status filter
        const matchesStatus = !filterState.documentRequests.status || 
            (filterState.documentRequests.status === record.status);

        return matchesSearch && matchesStatus;
    });

    // Re-apply current sort if exists
    const currentSortHeader = document.querySelector('#documentRequestsTable th span.asc, #documentRequestsTable th span.desc');
    if (currentSortHeader) {
        const columnIndex = parseInt(currentSortHeader.id.replace('documentRequestsTableHeader', ''));
        const isAsc = currentSortHeader.classList.contains('asc');
        
        filteredDocumentRequests.sort((a, b) => {
            // ... apply same sorting logic as in sortTable ...
            const props = ['reference', 'docId', 'title', 'borrower', 'dateReleased', 'status'];
            let aValue = a[props[columnIndex - 1]] || '';
            let bValue = b[props[columnIndex - 1]] || '';
...
// Document Requests Table Functions
function loadDocumentRequests() {
    // Example data - replace with actual API call
    const documentRequests = [
        {
...
function viewDocumentRequest(reference) {
    // Implement view functionality
    console.log('Viewing document request:', reference);
}

function markAsComplete(reference) {
    // Implement mark as complete functionality
    console.log('Marking as complete:', reference);
}

function deleteDocumentRequest(reference) {
    // Implement delete functionality
    console.log('Deleting document request:', reference);
}

// Add document requests table initialization to DOMContentLoaded
document.addEventListener('DOMContentLoaded', function() {
